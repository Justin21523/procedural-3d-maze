...
export class AutoPilot {
  ...
  tick(deltaTime) {
    ...
    // Next waypoint (skip current tile)
    const next = this.currentPath[0];
    // 如果已在 waypoint 或者距離 tile 中心很近，就往後推一個，避免在邊界抖動
    const targetCenter = {
      x: next.x * CONFIG.TILE_SIZE + CONFIG.TILE_SIZE / 2,
      z: next.y * CONFIG.TILE_SIZE + CONFIG.TILE_SIZE / 2
    };
    const dxWorld = targetCenter.x - this.playerController.position.x;
    const dzWorld = targetCenter.z - this.playerController.position.z;
    const distWorld = Math.hypot(dxWorld, dzWorld);
    if (
      (next.x === playerPos.x && next.y === playerPos.y && this.currentPath.length > 1) ||
      distWorld < CONFIG.TILE_SIZE * 0.2
    ) {
      this.currentPath.shift();
    }

    const target = this.currentPath[0] || next;
