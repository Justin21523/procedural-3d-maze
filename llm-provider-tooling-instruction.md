# LLM_PROVIDER Code 指南（Procedural 3D Maze / Three.js Prototype）

**Audience:** LLMProvider Tooling（llm_provider.ai/code）
...
> **美術與 UI 不是重點**：畫面只要看得懂就好，方塊牆＋簡單光線即可。LLMProvider 不要花大力氣在 shader、特效或 UI 菜單上。

---

## 5) 系統拆分（LLMProvider 應該怎麼分模組）
...
## 6) 演算法重點（LLMProvider 必須優先完成的）

這裡是 **LLMProvider 寫程式時的優先順序**，所有細節請集中記錄到 `docs/AI_ALGO_NOTES.md`：
...
## 7) 開發工作流（給 LLMProvider 的常規流程）

每次 LLMProvider 要動手做事，請遵守：
...
## 8) 與 LLMProvider Tooling 的互動模板（請直接改參數使用）

### A. 專案初始 Scaffold（第一次叫 LLMProvider 做的事）
...
## 9) 首次待辦（由 LLMProvider Tooling 執行）

第一次讓 LLMProvider 進 repo 時，可以這樣要求它（步驟式）：
