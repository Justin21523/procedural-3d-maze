...
export class Pathfinding {
   ...
   */
  findPath(start, goal, useCache = true) {
    // Check cache first
    if (useCache) {
  ...
  getNeighbors(pos) {
    ...
    const directions = [
      ...
      { x: 0, y: 1 },   // South
      { x: 0, y: -1 },  // North
      // Diagonal movement (optional, can be disabled for strict grid movement)
      { x: 1, y: 1 },   // Southeast
      { x: 1, y: -1 },  // Northeast
      { x: -1, y: 1 },  // Southwest
      { x: -1, y: -1 }, // Northwest
    ...
    for (const dir of directions) {
      ...
      if (this.worldState.isWalkable(neighbor.x, neighbor.y)) {
        ...
        if (dir.x !== 0 && dir.y !== 0) {
          const checkX = this.worldState.isWalkable(pos.x + dir.x, pos.y);
          const checkY = this.worldState.isWalkable(pos.x, pos.y + dir.y);
          if (checkX && checkY) {
